/**  版本信息模板在安装目录下，可自行修改。
* SignalData.cs
*
* 功 能： N/A
* 类 名： SignalData
*
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2018/5/4 8:53:36   N/A    初版
*
* Copyright (c) 2012 Maticsoft Corporation. All rights reserved.
*┌──────────────────────────────────┐
*│　此技术信息为本公司机密信息，未经本公司书面同意禁止向第三方披露．　│
*│　版权所有：动软卓越（北京）科技有限公司　　　　　　　　　　　　　　│
*└──────────────────────────────────┘
*/
using System;
namespace SignalR.Monitor.Model
{
	/// <summary>
	/// SignalData:实体类(属性说明自动提取数据库字段的描述信息)
	/// </summary>
	[Serializable]
	public partial class SignalData
	{
		public SignalData()
		{}
		#region Model
		private int _signalid;
		private string _signalname;
		private string _signalcontent;
		private string _signalmessage;
		private string _signalremark;
		/// <summary>
		/// 
		/// </summary>
		public int SignalID
		{
			set{ _signalid=value;}
			get{return _signalid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string SignalName
		{
			set{ _signalname=value;}
			get{return _signalname;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string SignalContent
		{
			set{ _signalcontent=value;}
			get{return _signalcontent;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string SignalMessage
		{
			set{ _signalmessage=value;}
			get{return _signalmessage;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string SignalRemark
		{
			set{ _signalremark=value;}
			get{return _signalremark;}
		}
		#endregion Model

	}
}

